...
def start(figure_id):
    ...
    field = rfidreaders.tags.index(figure_id) + 1
    ...
    while True:
        if (
            rfidreaders.tags[field] is None
            or record_timer < time.time()
            or "ENDE" in rfidreaders.tags
        ):
    ...
    waitingtime = (
        ...
        + float(
            subprocess.run(
                ["soxi", "-D", figure_dir + "/" + figure_id + ".mp3"],
