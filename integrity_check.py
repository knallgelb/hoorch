...
def get_assigned_entries():
    return {
        "actions": set(getattr(tag, "name", None) for tag in file_lib.actions_db.values()),
        "animals": set(getattr(tag, "name", None) for tag in file_lib.animal_figures_db.values()),
        "figures": set(getattr(tag, "name", None) for tag in file_lib.figures_db.values()),
        "games": set(getattr(tag, "name", None) for tag in file_lib.gamer_figures_db.values()),
        "numeric": set(getattr(tag, "number", None) for tag in file_lib.animal_numbers_db.values()),
    }
...
def find_missing_entries():
    ...
    missing = {cat: {val for val in expected[cat] if val and val not in assigned.get(cat, set())} for cat in expected}
    return missing
